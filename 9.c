...
int main() {
  ...
  while (fgets(str, 20, fp) != NULL) {
    ...
    for (k = i - 1; k >= 0; k--) {
      ...
      if (f2 == 0)
        if (strcmp(stmt[i].lc, stmt[k].lc) == 0 ||
            strcmp(stmt[i].lc, stmt[k].rc) == 0) {
          stmt[i].p1 = k;
          break;
        }
    }
    ...
    if (strcmp(stmt[i].rc, "\0") != 0)
      for (k = i - 1; k >= 0; k--) {
        ...
        if (f2 == 0)
          if (strcmp(stmt[i].rc, stmt[k].lc) == 0 ||
              strcmp(stmt[i].rc, stmt[k].rc) == 0) {
            stmt[i].p2 = k;
            break;
          }
      }
  ...
  for (k = 0; k <= i; k++) {
    if (stmt[k].op == '[')
      printf("\n\nAddress %d:: Node [] is created with %s", &stmt[k],
             stmt[k].lc);
    else
      printf("\n\nAddress %d:: Node %c is created with %s", &stmt[k],
             stmt[k].op, stmt[k].lc);
    if (stmt[k].p1 != -1)
